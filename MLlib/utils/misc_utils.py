...
class OneHotEncoder:
    ...
    #         It is used to calculate the number of unique values in
    #         each column and tell whether a particular column
    #         should be encoded or not.
    # (2) CHECK_TRANSFORM(INPUT_X) ---
    #                     It is used to check whether the data
    #                     which is being transformed has same
    #                     values as the data which was used
    #                     to fit it.
    # (3) TRANSFORM(INPUT_X) ---
    #               It is used to OneHotEncode the data
    #               based on the data which was used to fit
    # (4) FIT_TRANSFORM(INPUT_X, THRESHOLD) ---
    #                   This fuction is just a combination
    #                   of the fit and the transform fuction.

    # INPUTS
    # X - It is a numpy array of size n x m.
    # thresh - It is a threshold value which is calulated as
    #          THRESH = (NUMBER OF UNIQUE VALUES IN A COLUMN)/
    #                   (LENGTH OF COLUMN).
    #          Column whose threshold value is below the
    #          input threshold value which be encode otherwise not.
    ...
    # arr_nunique - It is an array which  is used to store the number of
    #               unique values in a particular column.
    # encode - It is an array of the size of the number of columns in the
    #          fit data. It has a value of 1 if the columns is to be
    #          encoded otherwise 0.
    ...
    def check_transform(self, X):
        ...
        if self.ncols != m:
            print(
                "Number of Columns in input data of fit function and \
        ...
        for i in range(m):
            ...
            if self.arr_nunique[i] != n_unique:
                print(
                    "Mismatch in the number of unique values in the "
                    + str(i)
                    + "th column"
                )
                return False
            for val in ls:
                if val not in self.arr_dic[i].keys():
                    print(
                        str(i)
                        + "th column contain a value which was not in \
                           the data used to fit data"
                    )
                    return False
